...
prci=100.0 # low values mean higher precision default : factr=10000000.0

def fitness(filename, lines_file = "x30.txt", error_rates = "error.rates.txt"):	
	def calc_v0(r_rates): 
	...
	def foward_backward(obs, states, start_p,transition_probability,er):
		...
		#print "ex",lnFactor,alpha[len(obs)-1]['AA'],alpha[len(obs)-1]['AB'],alpha[len(obs)-1]['BB']
	...
	R_rates=[1.0]  # recombination rate per bp (morgans / megabase) within scaffs
	for k in range(interScaff_intervals):
		R_rates.append(1.0) # recomb rates between scaffs
